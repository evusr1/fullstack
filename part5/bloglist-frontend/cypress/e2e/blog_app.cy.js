describe('Blog app', function() {
  ...
  describe('When logged in', function() {
    ...
    it('A blog can be created', function() {
      ...
      cy.get('.error').should('contain', 'a new blog Title of Blog by Product Productson')
    ...
    describe('and several blog posts', function() {
      ...
      it('remove button displays only to creator', function() {
        cy.contains('Title of Blog Product Productson').parent().as('notMineBlog')
        cy.get('@notMineBlog').contains('show').click()
        cy.get('@notMineBlog').contains('remove').should('have.css', 'display', 'none')

        cy.contains('The Cyber Baron').parent().as('MineBlog')
        cy.get('@MineBlog').contains('show').click()
        cy.get('@MineBlog').contains('remove').should('have.css', 'display', 'inline-block')
      ...
      it('test likes order', function() {
        ...
        cy.contains('Title of Blog Product Productson').parent().as('BlogWith1')
        ...
        cy.get('.blog').eq(1).should('contain', 'Title of Blog Product Productson')
